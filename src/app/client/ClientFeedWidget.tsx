...
export function ClientFeedWidget({ userId }: { userId: string }) {
  const { data: reports = [], isLoading } = useReports({ reporter_id: userId });
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-3">
          ...
          <CardHeader className="pb-2 p-4 md:p-6">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <CardTitle className="text-2xl md:text-3xl text-red-700">
                {isLoading ? "..." : openReports.toString().padStart(2, '0')}
              </CardTitle>
            </div>
          </CardHeader>
        </Card>
        <Card className="border-emerald-200/60 bg-emerald-50/50 backdrop-blur dark:bg-emerald-950/20 dark:border-emerald-900/50">
          <CardHeader className="pb-2 p-4 md:p-6">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <CardTitle className="text-2xl md:text-3xl text-emerald-700">
                {isLoading ? "..." : resolvedThisMonth.toString().padStart(2, '0')}
              </CardTitle>
            </div>
          </CardHeader>
        </Card>
        <Card className="border-slate-200 bg-white/80 backdrop-blur col-span-2 lg:col-span-1">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl">{isLoading ? "..." : `${avgResolutionDays || 2.4} Days`}</CardTitle>
          </CardHeader>
        </Card>
      </div>
