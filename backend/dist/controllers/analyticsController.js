...
exports.analyticsController = {
    ...
    async getSuperadminAnalytics(req, res) {
        try {
            ...
            const { data: statusCounts, error: statusErr } = await db_1.supabase
                .from('reports')
                .select('status');
            if (statusErr)
                throw statusErr;
            ...
            const { data: reportsWithIssueType, error: issueTypeErr } = await db_1.supabase
                .from('reports')
            ...
            const { data: recurringData, error: recErr } = await db_1.supabase
                .from('reports')
                .select(`
          issue_type,
        ...
        `);
            if (recErr)
                throw recErr;
            const { data: reportsOverTime, error: reportsOverTimeErr } = await db_1.supabase
                .from('reports')
                .select('created_at')
    ...
    async getAdminAnalytics(req, res) {
        try {
            ...
            const { data: reports, error } = await db_1.supabase
                .from('reports')
                .select(`
          status,
          issue_type,
